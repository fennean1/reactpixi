...
export default function LessonPanel(props) {
  ...
  const [fullscreen,setScreen] = React.useState(false)
...
const resize = ()=>{
  ...
  if (window.innerHeight/window.innerWidth < 0.275){
...
useEffect(()=> {
  ...
  function initButtons(){
    let buttons = [ <a className ="waves-effect grey waves-light btn" key = {2} style = {{margin: 2}} onClick = {()=>animate(-1)}><i className="material-icons">chevron_left</i></a>,<a key = {3} className ="waves-effect grey waves-light btn"  style = {{margin: 2}}  onClick = {()=>animate(1)}><i className="material-icons">chevron_right</i></a>]
    return buttons;
  }
    ...
    return (
      ...
      <div style = {{flex: 1,margin: 3}}>
        <a onClick = {()=> setMenuOpen(true)}className ="btn orange left"><i className="material-icons">menu</i></a>
      </div>
      ...
      <Document file="/pdfs/PanoramicTest.pdf">
            <Page height = {window.innerHeight*0.35} pageNumber={panelNumber} />
        </Document>
      </div>
        <div style = {{backgroundColor: 'red',flex: 1}}>
          <Arena fullscreen = {fullscreen} app = {app} script = {FractionNumberLineScript.init}/>
        </div>
      </div>
    );
} 
