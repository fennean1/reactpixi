...
export const init = (app, setup) => {
  ...
  function sendRowToBottom(row1, row2, start) {
    ...
    row1.forEach(t =>
      window.createjs.Tween.get(t)
        .to({ y: 12 * height }, 1000, window.createjs.Ease.getPowInOut(4))
        .call(swapCompleted)
    );
  }
  ...
  function swapRow(row, start, end) {
    if (Math.abs(start - end) < 30) {
      row.forEach(t =>
        window.createjs.Tween.get(t)
          .to({ y: start }, 1000, window.createjs.Ease.getPowInOut(4))
          .call(swapCompleted)
      );
    }
    // Moved up
      ...
      row.forEach(t =>
        window.createjs.Tween.get(t)
          .to({ y: roundedEnd }, 1000, window.createjs.Ease.getPowInOut(4))
          .call(swapCompleted)
      );
    }
    // Moved down
  ...
  function onDragMove() {
    if (this.dragging) {
      ...
      wallObj[this.denValue].forEach(t => {
        t.parent.addChild(t);
        t.y = newPosition.y;
      });
    }
